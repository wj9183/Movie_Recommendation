...
# test
# user_typed_title = 'asdasdfafsdasdf'
# st.write(sum(movie_id_titles_df.loc[movie_id_titles_df['title'].str.lower().str.contains(user_typed_title.lower()),]))
...
def recommend_movie():
    ...
    if select_menu == '취향에 맞는 영화로부터 추천받기':
        ...
        if sum(movie_id_titles_df['title'].str.lower().str.contains(user_typed_title.lower())) != 0:
            favorite_movie = movie_id_titles_df.loc[movie_id_titles_df['title'].str.lower().str.contains(user_typed_title.lower()) == True, 'title']
            favorite_movie = favorite_movie.iloc[0]
        else:
            pass
        ...
        if (len(user_typed_title) != 0) and (recommend_count != 0) and (least_review_counts != 0):
            if len(favorite_movie) != 0:
                ...
                st.write('검색하신 영화가 목록에 없어 추천할 수 없습니다.')
